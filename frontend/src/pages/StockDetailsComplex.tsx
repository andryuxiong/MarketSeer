...
const StockDetails: React.FC = () => {
  ...
  const formatLargeNumber = (num: number | undefined, suffix: string) => {
    if (num === undefined || num === null || isNaN(num)) return 'N/A';
    const divisor = suffix === 'B' ? 1e9 : 1e6; // Billion or Million
  ...
  useEffect(() => {
    const fetchStockData = async () => {
      ...
      try {
        ...
        console.log('Fetched stock data:', stockQuoteData);
        
        // Try to fetch profile data, but make it optional since the endpoint might not exist
        try {
          const profileRes = await fetch(formatApiUrl(`/api/stock/profile/${symbol}`));
          if (profileRes.ok) {
            const profileData = await profileRes.json();
            setCompanyProfile(profileData);
            console.log('Fetched company profile:', profileData);
          } else {
            console.log('Profile endpoint not available, using basic info from quote');
            // Create minimal profile from quote data
            setCompanyProfile({
              name: stockQuoteData.name || symbol || 'Unknown',
              ticker: symbol || 'N/A',
              weburl: '',
            });
          }
        } catch (profileError) {
          console.log('Profile fetch failed, using basic info:', profileError);
          setCompanyProfile({
            name: stockQuoteData.name || symbol || 'Unknown',
            ticker: symbol || 'N/A',
            weburl: '',
            ...
            <GridItem>
              ...
              >
                ...
                <Stat>
                  ...
                  <StatNumber fontSize="2xl" color={textColor}>${formatPrice(stockData?.o)}</StatNumber>
                </Stat>
              </Box>
            </GridItem>
            <GridItem>
              ...
              <GridItem>
                ...
                >
                  ...
                  <Text fontSize="2xl" color={textColor}>
                    {formatLargeNumber(companyProfile?.marketCapitalization, 'B')}
                  </Text>
                </Box>
              </GridItem>
              <GridItem>
                ...
                >
                  ...
                  <Text fontSize="2xl" color={textColor}>
                    {formatLargeNumber(companyProfile?.shareOutstanding, 'M')}
                  </Text>
                </Box>
              </GridItem>
              <GridItem colSpan={{ base: 1, md: 2 }}>
